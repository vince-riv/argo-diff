...
func checkArgoDiffAppProperties(a Application, t *testing.T) {
	...
	if a.Metadata.Namespace == "" {
		t.Error("Empty Namespace")
	}
	if a.Spec.Source.RepoURL == "" {
		t.Error("Empty RepoURL")
	}
	if a.Spec.Source.Path == "" {
		t.Error("Empty source path")
	}
	if a.Spec.Source.TargetRevision == "" {
		t.Error("Empty source revision")
	}
	if a.Spec.Destination.Server == "" {
		t.Error("Empty destination")
	}
	if a.Spec.Destination.Namespace == "" {
		t.Error("Empty destination namespace")
	}
	if a.Spec.Project == "" {
		t.Error("Empty project")
	}
	if a.Status.Sync.Status == "" {
		t.Error("Empty Sync Status")
	}
	if a.Status.Sync.Revision == "" {
		t.Error("Empty Sync Revision")
	}
	if a.Status.Health.Status == "" {
...
func TestLoadApplicationRefresh(t *testing.T) {
	...
	if a.Metadata.Name != "argo-diff" {
		t.Error("Name not argo-diff")
	}
	if a.Metadata.Namespace != "argocd" {
		t.Error("Namespace not argocd")
	}
	if a.Spec.Source.RepoURL != "ssh://git@github.com/vince-riv/argo-diff.git" {
		t.Error("Bad RepoURL")
	}
	if a.Spec.Source.Path != "k8s" {
		t.Error("Bad source path")
	}
	if a.Spec.Source.TargetRevision != "HEAD" {
		t.Error("Bad source revision")
	}
	if a.Spec.Destination.Server != "https://kubernetes.default.svc" {
		t.Error("Bad destination")
	}
	if a.Spec.Destination.Namespace != "argocd" {
		t.Error("Bad destination namespace")
	}
	if a.Spec.Project != "argocd-extras" {
		t.Error("Bad project")
	}
	if a.Status.Sync.Status != "OutOfSync" {
		t.Error("Bad Sync Status")
	}
	if a.Status.Sync.Revision != "0341d95b8c70dc5f555fc8fe337e14b5496ff092" {
		t.Error("Bad Sync Revision")
	}
	if a.Status.Health.Status != "Healthy" {
		t.Error("Bad Health Status")
	}
}

func TestLoadManifests(t *testing.T) {
	payload, _, err := readFileToByteArray(payloadManifests)
	if err != nil {
		t.Errorf("Failed to read %s: %v", payloadManifests, err)
	}
	m, err := decodeManifestsPayload(payload)
	if err != nil {
		t.Errorf("decodeManifestsPayload() failed: %s", err)
	}

	if len(m.Manifests) != 4 {
		t.Errorf("Expected 4 manitests")
	}
	for idx, mi := range m.Manifests {
		if mi == "" {
			t.Errorf("Manifest at list index %d is empty", idx)
		}
	}

	if m.Revision != "0341d95b8c70dc5f555fc8fe337e14b5496ff092" {
		t.Errorf("Revision %s doesn't match 0341d95b8c70dc5f555fc8fe337e14b5496ff092", m.Revision)
	}
	if m.SourceType != "Kustomize" {
		t.Errorf("SourceType %s doesn't match Kustomize", m.SourceType)
	}
}
...
func TestLoadErrorPayload(t *testing.T) {
	payload, _, err := readFileToByteArray(payloadError)
	if err != nil {
		t.Errorf("Failed to read %s: %v", payloadError, err)
	}
	errPayload := decodeErrorPayload(payload)

	if errPayload.Error == "Unknown" || errPayload.Error == "" {
		t.Errorf("decodeErrorPayload() failed - Error field is incorrect: %s", errPayload.Error)
	}
	if errPayload.Code < 1 {
		t.Errorf("decodeErrorPayload() failed - Code field is incorrect: %d", errPayload.Code)
	}
	if errPayload.Message == "Unknown" || errPayload.Message == "" {
		t.Errorf("decodeErrorPayload() failed - Message field is incorrect: %s", errPayload.Message)
	}
}
