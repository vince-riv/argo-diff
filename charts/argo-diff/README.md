...
| config.commentCollapseAppCount | string | `""` | In "auto" collapse mode, the application count above which a comment is folded. Must be positive. Defaults to 3 |
| config.commentCollapseResourceCount | string | `""` | In "auto" collapse mode, the changed-resource count above which an application's block is folded. Must be positive. Defaults to 5 |
