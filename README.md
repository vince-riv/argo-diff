...
| ARGO_DIFF_BYPASS_CONNECTIVITY_CHECKS | N/A                      | no               |          | Comma-separated list of startup connectivity checks to skip: `github`, `argocd`, or `true`/`all` for both. Useful when `GITHUB_TOKEN` holds a GitHub App installation token, which can't call `GET /user` (a per-PR CI invocation minting its own token, eg: from Jenkins, is the common case — GitHub Actions mode doesn't need this, it already skips the check). Bypassing `github` also makes argo-diff match its own prior PR comments by marker only, the same as it does under GitHub Actions — so two instances sharing an empty `ARGO_DIFF_CONTEXT_STR` on one PR will overwrite each other's comments. |
