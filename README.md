...
| ARGO_DIFF_COMMENT_COLLAPSE       | comment_collapse            | no               | `expanded` | Whether the collapsible sections in a PR comment start open. `expanded` (default) always starts them open; `collapsed` always starts them folded; `auto` keeps a small comment fully expanded and folds one that covers more than `ARGO_DIFF_COMMENT_COLLAPSE_APP_COUNT` applications, or an application with more than `ARGO_DIFF_COMMENT_COLLAPSE_RESOURCE_COUNT` changed resources. |
| ARGO_DIFF_COMMENT_COLLAPSE_APP_COUNT | comment_collapse_app_count | no           | `3`      | In `auto` collapse mode, the application count above which a comment is folded. Must be positive; a non-positive value warns and falls back to the default. |
| ARGO_DIFF_COMMENT_COLLAPSE_RESOURCE_COUNT | comment_collapse_resource_count | no | `5`      | In `auto` collapse mode, the changed-resource count above which an application's block is folded. Must be positive; a non-positive value warns and falls back to the default. |
