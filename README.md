...
### Example argo-diff Comments

You can view example comments generated by argo-diff in PRs opened in this repo:
- https://github.com/vince-riv/argo-diff/pull/333#issuecomment-5723228791
- https://github.com/vince-riv/argo-diff/pull/333#issuecomment-5723229593

Those comments are generated by a Github actions workflow that performs an end-to-end test of new builds.

The first comment demonstrates generic manifest changes, including a cascading app-of-apps change (`meta`)
that triggers downstream changes to a child application (`helm-deployment`).

The second comment shows the UX when the proposed manifests generate an error (`broken-deployment`).
