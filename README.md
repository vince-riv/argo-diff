...
| ARGO_DIFF_COMMENT_COLLAPSE       | comment_collapse            | no               | `auto`   | Whether the collapsible sections in a PR comment start open. `auto` (default) keeps a small comment fully expanded and folds one that covers more than 3 applications, or an application with more than 5 changed resources; `expanded` and `collapsed` force it either way. |
