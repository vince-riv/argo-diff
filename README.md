...
| ARGO_DIFF_TIMEOUT                | timeout                     | no               | `3m`     | How long argo-diff may spend processing a single event, as a Go duration (eg: `5m`, `90s`); a bare integer is treated as seconds. Raise this when a change matches many ArgoCD applications, since each one costs a round trip to the argocd server. Applications not diffed before the deadline are called out in the PR comment. |
